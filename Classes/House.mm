...
- (CGRect) getFrame
{
	...
	return CGRectMake(ipx((m_fX-76/2+fSwingXOffset)), ipy(m_fY), ipx(76), ipy(58));
}
...
- (CGRect) getAbsoluteFrame
{
	...
	return CGRectMake(ipx((m_fX-76/2+fSwingXOffset)), ipy((m_fY+fBaseY)), ipx(76), ipy(58));
}
...
- (void) draw: (float) x y:(float) y fAlpha:(float) fAlpha
{
	...
	CGPoint point = [self calcCB: ccp(ipx(x), ipy(y)) sprite: sprite];
...
- (void) drawCrashEffect1
{
	...
	{
		[self drawSpriteToCB: @"bcrashleft1" point: ccp(ipx((m_fX-35)), ipy((m_fY+fBaseY)))];
		[self drawSpriteToCB: @"bcrashright1" point: ccp(ipx((m_fX+35)), ipy((m_fY+fBaseY)))];
	}
	else if (fCrashCycle >= 6 && fCrashCycle <= 10)
	{
		[self drawSpriteToCB: @"bcrashleft2" point: ccp(ipx((m_fX-40)), ipy((m_fY+fBaseY)))];
		[self drawSpriteToCB: @"bcrashright2" point: ccp(ipx((m_fX+40)),ipy(( m_fY+fBaseY)))];
	}
	else if (fCrashCycle >= 11 && fCrashCycle <= 15)
	{
		[self drawSpriteToCB: @"bcrashleft3" point: ccp(ipx((m_fX-45)), ipy((m_fY+fBaseY)))];
		[self drawSpriteToCB: @"bcrashright3" point: ccp(ipx((m_fX+45)), ipy((m_fY+fBaseY)))];
	}
	else if (fCrashCycle >= 16 && fCrashCycle <= 20)
	{
		[self drawSpriteToCB: @"bcrashleft4" point: ccp(ipx((m_fX-50)), ipy((m_fY+fBaseY)))];
		[self drawSpriteToCB: @"bcrashright4" point: ccp(ipx((m_fX+50)), ipy((m_fY+fBaseY)))];
	}
}
...
- (void) drawCrashEffect2
{
	...
	if (fCrashCycle <= 50)
	{
		[self drawAngleSpriteToCB: @"btink3" point: ccp(ipx((m_fX-35)), ipy((m_fY+fBaseY))) angle: fCrashCycle];
		[self drawAngleSpriteToCB: @"btink4" point: ccp(ipx((m_fX+35)), ipy((m_fY+fBaseY))) angle: fCrashCycle];
	}
	
	if (fCrashCycle >= 40 && fCrashCycle <= 100)
	{
		[self drawAngleSpriteToCB: @"btink3" point: ccp(ipx(m_fX), ipy((m_fY+fBaseY+20))) angle: fCrashCycle];
		[self drawAngleSpriteToCB: @"btink1" point: ccp(ipx((m_fX-35)),ipy(( m_fY+fBaseY))) angle: fCrashCycle];
		[self drawAngleSpriteToCB: @"btink1" point: ccp(ipx((m_fX+35)),ipy(( m_fY+fBaseY+40))) angle: fCrashCycle];
	}
}
...
- (void) drawBubbleEffect
{
	...
	if (m_aniEffect.m_nCurFrame <= 20)
	{
		...
		[sprite setPosition: ccp(ipx(m_fX), ipy((fBaseY+m_fY)))];
	...
	[self drawAngleSpriteToCB: @"b134" point: ccp(ipx(x), ipy((fBaseY+y))) angle: 2.5*fCrashCycle];

	x = m_fX+cos(-PI/3.0f) * fCrashCycle;
	y = m_fY + sin(-PI/3.0f)*fCrashCycle/2;		
	[self drawAngleSpriteToCB: @"b135" point: ccp(ipx(x), ipy((fBaseY+y))) angle: 0.8*fCrashCycle];

	x = m_fX+cos(-PI/2.0f) * (fCrashCycle-20);
	y = m_fY + sin(-PI/2.0f)*(fCrashCycle-20)/2;		
	[self drawAngleSpriteToCB: @"b136" point: ccp(ipx(x), ipy((fBaseY+y))) angle: 1.2*fCrashCycle];
	
	x = m_fX+cos(-PI*2/3.0f) * (fCrashCycle-10);
	y = m_fY + sin(-PI*2/3.0f)*(fCrashCycle-10)/2;		
	[self drawAngleSpriteToCB: @"b136" point: ccp(ipx(x), ipy((fBaseY+y))) angle: 0.5*fCrashCycle];

	x = m_fX+cos(-PI) * (fCrashCycle+10);
	y = m_fY + sin(-PI)*(fCrashCycle+10)/2;		
	[self drawAngleSpriteToCB: @"b136" point: ccp(ipx(x), ipy((fBaseY+y))) angle: 0.6*fCrashCycle];

	x = m_fX+cos(-PI/2.5f) * (fCrashCycle-15);
	y = m_fY + sin(-PI/2.5f)*(fCrashCycle-15)/2;		
	[self drawAngleSpriteToCB: @"b152" point: ccp(ipx(x), ipy((fBaseY+y))) angle: 0.6*fCrashCycle];

	x = m_fX+cos(-PI/4.5f) * (fCrashCycle-25);
	y = m_fY + sin(-PI/4.5f)*(fCrashCycle-15)/2;		
	[self drawAngleSpriteToCB: @"b153" point: ccp(ipx(x), ipy((fBaseY+y))) angle: 0.6*fCrashCycle];
	...
	[self drawAngleSpriteToCB: @"b154" point: ccp(ipx(x), ipy((fBaseY+y))) angle: 0.6*fCrashCycle];

	x = m_fX+cos(-3*PI/2.0f) * (fCrashCycle-5);
	y = m_fY + sin(-3*PI/2.0f)*(fCrashCycle-5)/2;		
	[self drawAngleSpriteToCB: @"btink3" point: ccp(ipx(x), ipy((fBaseY+y))) angle: 0.6*fCrashCycle];
...
- (BOOL) visibleInViewport
{
	...
	CGRect rtScreen = CGRectMake(0, 0, ipx(320), ipy(480));
...
//			[spriteChild setRotation: sin(fSwingAngle)*m_fSwingWidth/5];
	...
	CGPoint point = [self calcCB: ccp(ipx((m_fX+fOffsetX)), ipy((m_fY+fBaseY+fOffsetY))) sprite: sprite];
	[sprite setPosition: point];
	[sprite visit];
