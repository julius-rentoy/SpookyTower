...
-(id) init
{
	...
	if( (self=[super init] )) 
	{
        [[SoundManager sharedSoundManager] playBackgroundMusic: kBGame];
		...
		mSmallLabel = [[CCLabelFX labelWithString:@"test" 
								  dimensions:CGSizeMake(ipx(100), ipy(30)) 
								   alignment:CCTextAlignmentCenter
									fontName:@"Arial" 
									fontSize:ipx(15)
								shadowOffset:CGSizeMake(0,0) 
		...
		CCButton* btnClose = [[CCButton alloc] initWithTarget:self 
											   ...
											   selTextureName: @"pause1"
														text : @""
											position:CGPointMake(ipx(260), ipy(20))];
		mbtnPause = btnClose;
		...
		CC3PartButton* btnResume = [[CC3PartButton alloc] initWithTarget:self 
														  ...
														  selTextureName: @"option_btn_resume1"
																	text: @""
																position: CGPointMake(ipx(125), ipy(400))];
		...
		CC3PartButton* btnMenu = [[CC3PartButton alloc] initWithTarget:self 
														  ...
														  selTextureName: @"option_btn_menu1"
																	text: @""
																position: CGPointMake(ipx(125), ipy(430))];
...
- (void) submitMyScore
{
    ...
    towerGameAppDelegate* app = (towerGameAppDelegate*)[UIApplication sharedApplication].delegate;
    [app addOne];
}
...
- (void) createMusicSlider
{
	...
	slider.position = ccp( ipx(158), ipy(246) );
...
- (void) createSFXSlider
{
	...
	slider.position = ccp( ipx(158), ipy(146) );
	slider.liveDragging = YES;
...
- (void) drawRope
{
	...
	ccDrawLine(ccp(ipx((x-1)), ipy(y)), ccp(ipx((158)), ipy(600)) );
    
	glLineWidth(2);
	glColor4ub(0x80, 0x50, 0x20,0xFF);	
	ccDrawLine(ccp(ipx(x), ipy(y)), ccp(ipx(160), ipy(600)));
	
	glLineWidth(1);
	glColor4ub(0xBF, 0x8F, 0x80, 0xFF);	
	ccDrawLine(ccp(ipx((x+1)), ipy(y)), ccp(ipx(162), ipy(600)));
	
	glLineWidth(1);
	glColor4ub(0xFF, 0xFF, 0xFF, 0xFF);	
    
	[self drawSpriteToCenter: @"b16" position: ccp(ipx(x), ipy(y))];
	
	if ([mHouse m_bFall] == YES || mHouse.hidden)
		[self drawSpriteToCenter: @"b74" position: ccp(ipx((x+1)), ipy((y-18)))];
	
	//Shader Draw
	if ([mApartment count] == 0)
		[self drawSpriteToCenter: @"c25" position: ccp(ipx(x), ipy(50))];
...
- (void) drawStatus
{
	[self drawSprite: @"c16" position: CGPointMake(ipx(4), ipy(430))];

	[mLabel setString: [NSString stringWithFormat: @"%d", [mApartment count]]];
	[mLabel setPosition:ccp(ipx(35), ipy(442))];
	[mLabel visit];
	...
	{
		[self drawSprite: @"b80" position: CGPointMake(ipx(28), ipy((fOffsetY-fDiff*i)))];
	}
}
...
- (void) procGameOver
{
    ...
    [[towerGameAppDelegate getInstance] uploadScore:dialog.m_nFloor
                                       withGameType:(int)m_nGameType];
...
- (void) drawPauseMenu
{
	...
	[self drawSprite: @"text_pause" position: ccp(ipx(80), ipy(320))];
...
- (void) drawTime
{
	...
	[sprite setPosition: ccp(ipx(fBaseX), ipy(430))];
	[sprite visit];

	nTime = (lRemain / 1000) % 10;
	sprite = [resManager getTextureWithName: rNumImage[nTime]];
	[sprite setPosition: ccp(ipx((fBaseX+20)), ipy(430))];
	[sprite visit];

	sprite = [resManager getTextureWithName: @"bcomma"];
	[sprite setPosition: ccp(ipx((fBaseX+40)), ipy(430))];
	[sprite visit];
	
	nTime = (lRemain / 100) % 10;
	sprite = [resManager getTextureWithName: rNumImage[nTime]];
	[sprite setPosition: ccp(ipx((fBaseX+60)), ipy(430))];
	[sprite visit];

	nTime = (lRemain / 10) % 10;
	sprite = [resManager getTextureWithName: rNumImage[nTime]];
	[sprite setPosition: ccp(ipx((fBaseX+80)), ipy(430))];
	[sprite visit];
}
...
- (void) drawScore
{
	...
	nScore = lScore % 10;
	sprite = [resManager getTextureWithName: rNumImage[nScore]];
	[sprite setPosition: ccp(ipx((fBaseX+100)), ipy(430))];
	[sprite visit];
}
...
{towerGameAppDelegate *mainDelegate = (towerGameAppDelegate *)[[UIApplication sharedApplication]delegate];
    ...
    if(m_nGameType == GAMETYPE_QUICK) {
        mainDelegate.m_showhint = 90;
    }else if(m_nGameType == GAMETYPE_SCOREATTACK) {
        mainDelegate.m_showhint = 91;
    }else if(m_nGameType == GAMETYPE_TIMEATTACK) {
        mainDelegate.m_showhint = 92;
    } 
...
- (void) showIAPNOADS :(id)sender 
{
    /*
	//if([MKStoreManager isFeaturePurchased: featureAIdVar] == NO) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Notice" message:@"Turn off banner adds, upgrade to full version only $0.99 via in-app purchase.Do you want to continue?" delegate:self cancelButtonTitle:@"NO" otherButtonTitles:@"YES", nil];
        alert.tag = 10;
        [alert show];
        [alert release];
    //}
...
- (void) createAd {
    ...
    [[iaAdsManager  sharedAdsManager] newAd:ccp(0, 0)];
    ...
    [m_btnMenuAd setPosition:ccp( SCREEN_WIDTH-sprite1.contentSize.width ,sprite1.contentSize.height * (ipxbutton(2.2)))];
...
/*    
    ...
    if([MKStoreManager isFeaturePurchased: featureAIdVar] == NO ) {
        ...
        [[iaAdsManager sharedAdsManager] showAd];
