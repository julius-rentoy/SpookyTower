...
#import "GameConfig.h"
#define kClippingMultiplier 1.0f//(kIsDeviceIPAD) ? (1.0f) : (1.0f/CC_CONTENT_SCALE_FACTOR())

//static const int _max = iphoneipadmax(); // bar.png Image Width
...
{
	[super initFromNormalImage:SHImageString(@"slider_blue_bar") 
				 selectedImage:SHImageString(@"slider_blue_bar" )
...
-(id) initWithTarget:(id)t selector:(SEL)sel
{
	if( (self = [super init]) ) {
		...
		if( _touchLogic ) {
			...
			barImage_ = [CCSprite spriteWithFile:SHImageString(@"slider_blue_bar")];
			[node setClippingRegion:CGRectMake(self.position.x - (barImage_.contentSize.width/2 * kClippingMultiplier),
											   self.position.y - (barImage_.contentSize.height/2 * kClippingMultiplier),
											   barImage_.contentSize.width * kClippingMultiplier,
											   barImage_.contentSize.height * kClippingMultiplier)];
