...
- (id) init
{
	if ( (self=[super init]) )
	{
		...
		renderTexture = [[CCRenderTexture renderTextureWithWidth:ipx(100) height:ipy(100)] retain]; 
...
- (void) drawFrame: (CGPoint) posCenter
{
	...
	for (unsigned int i = 0; i < [aryKeys count]; i ++)
	{
		...
		[sprite setPosition: ccp(ipx((50+x+contentSize.width/2)), ipy((100-(50+y+contentSize.height/2))))];
		[sprite visit];
	}
