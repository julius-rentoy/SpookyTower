...
{
	return [m_sqlManager lookupAllForSQL: @"select * from tbl_score order by score desc limit 10"];
}
...
- (void) resetScore
{
	[m_sqlManager runDynamicSQL: @"delete from tbl_score" forTable: @"tbl_score"];	
...
- (void) submitScore: (NSString*) scoreName score: (int) score  
{
	...
	[strSQL appendFormat: @"insert into '%@'('name', 'score', 'add_date') values(", @"tbl_score"];
...
{
	NSArray* scoreArray = [m_sqlManager lookupAllForSQL: [NSString stringWithFormat: @"select * from tbl_score where score>%d", score]];
